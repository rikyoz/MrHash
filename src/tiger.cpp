...
void Tiger::round(uint64_t * ra, uint64_t * rb, uint64_t * rc, uint64_t x,
		  int mul)
{
	uint64_t a = *ra;
	uint64_t b = *rb;
...
void Tiger::pass(uint64_t * ra, uint64_t * rb, uint64_t * rc, uint64_t * x,
		 int mul)
{
	uint64_t a = *ra;
	uint64_t b = *rb;
 ...
 */
void Tiger::tiger_write(const byte * inbuf, size_t inlen)
{
 ...
 */
byte * Tiger::tiger_final()
{
	uint32_t t, msb, lsb;
	byte *p;

	tiger_write(NULL, 0); /* flush */
...
{
	string test1("");
	string test1_md("24F0130C63AC933216166E76B1BB925FF373DE2D49584E7A");
	string test2("abc");
	string test2_md("F258C1E88414AB2A527AB541FFC5B8BF935F7B951C132951");
	string test3("Tiger");
	string test3_md("9F00F599072300DD276ABB38C8EB6DEC37790C116F9D2BDF");
	string test4("ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefgh"
		     "ijklmnopqrstuvwxyz0123456789+-");
	string test4_md("87FB2A9083851CF7470D2CF810E6DF9EB586445034A5A386");
	string test5("Tiger - A Fast New Hash Function, "
		     "by Ross Anderson and Eli Biham, proc"
		     "eedings of Fast Software Encryption "
		     "3, Cambridge, 1996.");
	string test5_md("3D9AEB03D1BD1A6357B2774DFD6D5B24DD68151D503974FC");

	Tiger tiger;

	if (unlikely(tiger.calcTiger(test1) != test1_md))
		return false;
	if (unlikely(tiger.calcTiger(test2) != test2_md))
		return false;
	if (unlikely(tiger.calcTiger(test3) != test3_md))
		return false;
	if (unlikely(tiger.calcTiger(test4) != test4_md))
		return false;
	if (unlikely(tiger.calcTiger(test5) != test5_md))
		return false;

	return true;
}

string Tiger::calcTiger(const string &buf)
{
	byte *ret;

	tiger_init();
	tiger_write(reinterpret_cast<const byte *>(buf.c_str()),
		    buf.length());
	ret = tiger_final();
